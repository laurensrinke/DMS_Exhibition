...
texture TexDiff <string uiname="DiffusionTexture";>;
sampler SampDiff = sampler_state    //sampler for doing the texture-lookup
{
    Texture   = (TexDiff);          //apply a texture to the sampler
    MipFilter = LINEAR;         //sampler states
    MinFilter = LINEAR;
    MagFilter = LINEAR;
};
...
float4 PS(vs2ps In): COLOR
{
    ...
    float4 col = tex2D(Samp, In.TexCd) * cAmb * saturate((tex2D(SampDiff, In.TexCd) / DiffPow));
    col = mul(col, tColor);
	col.a *= Alpha;
    return col;
}
